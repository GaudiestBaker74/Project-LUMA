...
void MainLoopFramework::waitForRetrace() {
    ...
    static bool sLoggedFirstRetrace = false;
    if (!sLoggedFirstRetrace) {
        sLoggedFirstRetrace = true;
        PL_LOG_INFO("boot", "waitForRetrace: first retrace received — the frame "
                            "loop is cycling on the VI field clock");
    }
...
namespace {
    ...
    void handleGXAbortAlarm(OSAlarm* alarm, OSContext* context) {
        ...
        GXDisableBreakPt();
        GXAbortFrame();
        // PC_PORT PATCH: MWERKS float literals ("0f"/"1f") are invalid C++
// on host toolchains; normalized to "0.0f"/"1.0f". Also:
// PC_PORT PATCH (compat/patches): guard the null sInstance. On the
        // host the emulated CP is synchronous, the alarm is cancelled right
        // after GXDrawDone and this handler never runs; the guard keeps the
        // watchdog path safe should a cancellation ever be lost.
