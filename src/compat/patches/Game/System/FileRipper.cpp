// =============================================================================
// PC_PORT PATCH of the vendored Game/System/FileRipper.cpp (see
// patches/README.md).
//
// Change: a pointer->u32 cast goes through uintptr_t (64-bit host pointers).
// =============================================================================
...
void* FileRipper::loadToMainRAM(const char* fpath, u8* dest, bool decompress, JKRHeap* pHeap, AllocDirection allocDir) {
    ...
    if (decompress) {
        u8 buf[0x60];
        // PC_PORT: the original `(u32)(buf + 0x3f) / 0x40 * 0x40` aligns a
        // 64-bit host pointer using 32-bit arithmetic (as on the Wii); keep
        // the u32 domain via uintptr_t so the alignment is identical.
        copySrc = reinterpret_cast<u8*>(static_cast<u32>((static_cast<u32>(reinterpret_cast<uintptr_t>(buf)) + 0x3f) / 0x40) * 0x40);
        while (true) {
