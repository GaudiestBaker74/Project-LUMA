...
| `nw4r/lyt/lyt_material.cpp` | **M9.5.3c**: validación de los offsets de textura en el ctor de `Material` — `texIdx` contra `texNum` del txl1 y `nameStrOffset` contra el tamaño del bloque (la consola interpretaba el offset relativo al array de entradas en `block+12`; un brlyt con entradas corruptas producía un puntero salvaje → SIGSEGV en `ResTable::getRes` durante `Layout::Build`, reproducido con un arc sintético). Degradación: nombre vacío → textura no encontrada → el fallback blanco de `flushDraw`; el slot SIGUE contando (`di`) para no desalinear los índices `texMap` de los TevStages. Además, guard de `pTextureList == nullptr`. |
