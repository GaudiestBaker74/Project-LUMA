// PC_PORT PATCHED COPY of petari/libs/nw4r/src/nw4r/ut/ut_TextWriterBase.cpp.
// Adds the two decomp omissions that the explicit instantiations at the bottom
// of this TU force into the link:
//   * TextWriterBase<T>::PrintImpl   (declared in the header, never defined)
//   * TextWriterBase<T>::mDefaultTagProcessor (static member definition)
// Everything else is byte-identical to the vendored file.
...
namespace nw4r {
    namespace ut {
        ...
        f32 TextWriterBase< CharType >::PrintImpl(StreamType str, int length) {
            // PC_PORT reconstruction: walks the stream like CalcLineRectImpl
            // does, but emits each character through CharWriter::Print and
            // routes control codes (< ' ') through the tag processor.
            const StreamType end = str + length;
            PrintContext< CharType > context = {this, str, 0, 0, 0};
            f32 width = 0;
            bool bCharSpace = false;
            ...
            for (CharCode code = reader.Next();
                ...
                if (code < ' ') {
                    ...
                    if (bCharSpace) {
                        SetCursorX(GetCursorX() + GetCharSpace());
                    }

                    CharWriter::Print(code); // qualified: Print(StreamType,int) hides it
                    bCharSpace = true;
                }

                width = Max(width, GetCursorX());
            }

            return width;
        }

        // PC_PORT: the static member's definition. The explicit instantiations
        // below emit it as a linkonce symbol once the primary-template
        // definition is visible in this TU (GCC silently ignores an
        // initializer-less `template<>` explicit-specialization definition).
        template < typename CharType >
