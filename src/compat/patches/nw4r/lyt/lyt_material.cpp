// =============================================================================
// PC_PORT PATCH of the vendored nw4r/lyt/lyt_material.cpp (see
// src/compat/patches/README.md).
//
// Change vs. upstream (M9.5.3c hardening): Material::Material resolved each
// texture as `ConvertOffsToPtr<char>(textures, textures[texIdx].nameStrOffset)`
// with NO validation. In a brlyt whose txl1 offsets/indices are corrupt (or
// merely written with the wrong base by a tool), nameStrOffset is an
// attacker/garbage-controlled u32: the resulting pointer is wild and
// LayoutHolder::GetResource -> ResTable::getRes -> MR::getHashCodeLower
// dereferences it -> SIGSEGV during Layout::Build (reproduced with a
// synthetic WiiRemoteStrap.arc). On the console this was "safe" because the
// game only ever shipped valid archives; on PC, assets are user-provided, so
// validate texIdx against the txl1 count and nameStrOffset against the txl1
// block size, and degrade to the same "texture not found" path as upstream
// (ReplaceImage(nullptr) is already guarded -> the draw falls back to the
// 1x1 white texture) instead of crashing.
...
#pragma dont_inline on
        ...
        Material::Material(const res::Material* pRes, const ResBlockSet& resBlockSet) {
            ...
            if (mpGXMem) {
                ...
                // PC_PORT: pTextureList can be null (material with texMaps in a
                // brlyt without txl1); upstream would deref null below.
                if (texMapNum > 0 && resBlockSet.pTextureList != nullptr) {
                    ...
                    // PC_PORT: validated bounds for the name lookup (see the
                    // file-header note). An out-of-range entry degrades to the
                    // "texture not found" path (empty name -> GetResource null
                    // -> guarded ReplaceImage), it does NOT advance past the
                    // slot: di still counts it so TevStage texMap indices stay
                    // aligned, exactly like a TPL that fails to load.
                    const u32 texListNum = resBlockSet.pTextureList->texNum;
                    const u32 texListSize = resBlockSet.pTextureList->blockHeader.size;
                    const bool texListOk =
                        texListSize >= sizeof(*resBlockSet.pTextureList);
                    const u32 texListNameLimit =
                        texListOk ? texListSize - sizeof(*resBlockSet.pTextureList) : 0;

                    u8 di = 0;
                    for (u8 si = 0; si < texMapNum; ++si) {
                        const char* fileName = "";
                        const u16 texIdx = pResTexMap[si].texIdx;
                        if (!texListOk || texIdx >= texListNum ||
                            textures[texIdx].nameStrOffset >= texListNameLimit) {
                            PL_LOG_WARN("compat.lyt",
                                        "Material '%s': texMap %u out of range "
                                        "(texIdx %u, texNum %u, block size %u) — texture skipped",
                                        GetName(), static_cast< unsigned >(si),
                                        static_cast< unsigned >(texIdx),
                                        static_cast< unsigned >(texListNum),
                                        static_cast< unsigned >(texListSize));
                        } else {
                            fileName = detail::ConvertOffsToPtr< char >(textures, textures[texIdx].nameStrOffset);
                        }
