// =============================================================================
// PC_PORT PATCH of the vendored nw4r/lyt/lyt_window.cpp (see
// src/compat/patches/README.md).
//
// Change vs. upstream (M9.5.4 hardening): the Window resource constructor
// indexed the mat1 offset table with the content's and every frame's
// `materialIdx` unconditionally. A brlyt whose wnd1 block precedes mat1
// (null pMaterialList) or carries an out-of-range index (corrupt /
// mis-swapped data) read a wild offset and faulted inside the Material ctor
// — silently, on the scene-init worker thread. The patched ctor validates
// every index against MaterialList::materialNum first: an invalid content
// index leaves the window without a content material (DrawContent now bails
// on it), an invalid frame index drops the whole frame set (mFrameNum = 0),
// because the rest of the class dereferences mFrames[i].pMaterial
// unconditionally. Same guard TextBox/Picture got in their patched files.
//
...
namespace nw4r {
    namespace lyt {
        ...
        Window::Window(const res::Window* pRes, const ResBlockSet& rBlockSet) : Pane(pRes) {
            ...
            // PC_PORT (M9.5.4 hardening): every materialIdx below is checked
            // against mat1 before the offset table is indexed (file header).
            const u32 materialNum = rBlockSet.pMaterialList != NULL ? rBlockSet.pMaterialList->materialNum : 0u;
            const u32* const pMatOffsetTbl =
                rBlockSet.pMaterialList != NULL ? detail::ConvertOffsToPtr< u32 >(rBlockSet.pMaterialList, sizeof(res::MaterialList)) : NULL;
            ...
            if (pResContent->materialIdx < materialNum) {
                ...
                // PC_PORT (M9.5.4 hardening): no content material (DrawContent bails).
                PL_LOG_WARN("compat.lyt", "Window '%.16s': content material %u out of range (materialNum %u) — no material",
                            pRes->name, static_cast< unsigned >(pResContent->materialIdx), static_cast< unsigned >(materialNum));
            }

            mFrameNum = 0;
            mFrames = NULL;

            // PC_PORT (M9.5.4 hardening): pre-validate every frame's
            // materialIdx; one bad frame drops the whole frame set (the rest
            // of the class dereferences mFrames[i].pMaterial unconditionally).
            bool framesOk = pRes->frameNum > 0;

            if (framesOk) {
                const u32* const pFrmOffsetTbl = detail::ConvertOffsToPtr< u32 >(pRes, pRes->frameOffsetTableOffset);

                for (int i = 0; i < pRes->frameNum; i++) {
                    const res::WindowFrame* const pResFrame = detail::ConvertOffsToPtr< res::WindowFrame >(pRes, pFrmOffsetTbl[i]);

                    if (pResFrame->materialIdx >= materialNum) {
                        PL_LOG_WARN("compat.lyt", "Window '%.16s': frame %d material %u out of range (materialNum %u) — frames dropped",
                                    pRes->name, i, static_cast< unsigned >(pResFrame->materialIdx), static_cast< unsigned >(materialNum));
                        framesOk = false;
                        break;
                    }
                }
        ...
        void Window::DrawContent(const math::VEC2& rBase, const WindowFrameSize& rFrameSize, u8 alpha) {
            // PC_PORT (M9.5.4 hardening): a window whose content material was
            // rejected by the ctor keeps drawing its frames, not its content.
            if (mpMaterial == NULL) {
                return;
            }
