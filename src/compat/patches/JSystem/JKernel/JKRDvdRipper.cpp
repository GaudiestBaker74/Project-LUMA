// =============================================================================
// PC_PORT PATCH of the vendored JSystem/JKernel/JKRDvdRipper.cpp (see
// patches/README.md).
//
// M9.5.1: DVD→RAM loader with on-the-fly Yaz0 decoding — what JKRMemArchive
// uses to pull a whole .arc/.arc.szs into memory.
//
// Changes vs. upstream:
//
// 1. **Endianness.** `decompSZS_subroutine` read the Yaz0 header through
//    `SYaz0Header* header = (SYaz0Header*)src; header->length` — a plain
//    big-endian u32 load on the PPC console, a byteswapped value on x86-64/
//    ARM64. The end pointer was computed from that garbage length, so
//    streaming-decoded archives would overflow/underflow the destination.
//    The length is now read big-endian explicitly.
//
// 2. **Deterministic read errors.** The retry loops treat any negative
//    DVDReadPrio result other than -3 as retryable (sleep a retrace, read
//    again) — correct for a real drive, but the host DVD layer fails
//    deterministically (missing file, out-of-bounds) and returns -1, which
//    would spin forever. Any negative result is now fatal for the load.
//
...
inline int decompSZS_subroutine(u8*, u8*);
...
void* JKRDvdRipper::loadToMainRAM(s32 entryNumber, u8* dst, JKRExpandSwitch expandSwitch, u32 dstLength, JKRHeap* heap,
    ...
    if (!file.open(entryNumber)) {
        PL_LOG_WARN("compat.rarc", "JKRDvdRipper: cannot open DVD entry %d", (int)entryNumber);
        return nullptr;
    }
    return JKRDvdToMainRam(&file, dst, expandSwitch, dstLength, heap, allocDirection, offset, pCompression, param_8);
}
...
void* JKRDvdRipper::loadToMainRAM(JKRDvdFile* dvdFile, u8* dst, JKRExpandSwitch expandSwitch, u32 dstLength, JKRHeap* heap,
    ...
    if (expandSwitch == EXPAND_SWITCH_UNKNOWN1) {
        ...
        while (true) {
            ...
            // PC_PORT: the host DVD layer fails deterministically (-1), so
            // retrying would spin forever; treat any error as fatal.
            (void)errorRetry;
            return NULL;
        }
    ...
    if (compression == COMPRESSION_NONE) {
        ...
        if (offset != 0) {
            u8 buffer[0x40];
            u8* bufPtr = (u8*)ALIGN_NEXT((uintptr_t)buffer, 32);
            // PC_PORT: no retry loop — host DVD failures are deterministic.
        ...
        while (true) {
            ...
            // PC_PORT: deterministic host failures — any error is fatal.
            if (hasAllocated == true) {
                JKRFree(dst);
            }
...
// this won't inline
// shrug
inline int decompSZS_subroutine(u8* src, u8* dest) {
    u8* endPtr;
    s32 validBitCount = 0;
    s32 currCodeByte = 0;
    u32 ts = 0;
    ...
    // PC_PORT: the Yaz0 header stores the decompressed size big-endian. The
    // console read it through the struct pointer (PPC is big-endian); on the
    // host that load byteswaps it, so read the 4 bytes explicitly.
    const u32 declaredSize = (static_cast<u32>(src[4]) << 24) | (static_cast<u32>(src[5]) << 16) |
                             (static_cast<u32>(src[6]) << 8) | static_cast<u32>(src[7]);
    endPtr = dest + (declaredSize - fileOffset);
