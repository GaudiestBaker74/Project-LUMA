// =============================================================================
// PC_PORT PATCH of the vendored JSystem/JKernel/JKRSolidHeap.cpp (see
// patches/README.md).
//
// Change: JKRSolidHeap::do_free(void*) is declared by the vendored header
// (vtable slot) but not implemented in the vendored sources — the linker
// needs the address when the class is instantiated. A solid heap never frees
// individual blocks (only freeAll), so the implementation is empty; this is
// also what the function does on the console in practice (the block allocator
// behind it doesn't reclaim). Everything else is the vendored file.
// =============================================================================
...
// PC_PORT: vtable slot implemented (see the header comment).
void JKRSolidHeap::do_free(void* ptr) {
    (void)ptr; // solid heaps only free in bulk (do_freeAll)
}
