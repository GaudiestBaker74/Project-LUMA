// =============================================================================
// PC_PORT PATCH of the vendored JSystem/JAudio2/JASHeapCtrl.cpp (see
// patches/README.md).
//
// Changes (documented; nothing else differs from the vendored file):
//   - pointer->u32 casts in JASHeap::initRootHeap / JASHeap::alloc are
//     computed through uintptr_t (64-bit host pointers); the resulting u32
//     offsets are the same arithmetic as on the Wii.
// =============================================================================
...
void JASHeap::initRootHeap(void* param_0, u32 param_1) {
    ...
    mHeap = nullptr;
    mSize = param_1 - static_cast<u32>(reinterpret_cast<uintptr_t>(mBase) - reinterpret_cast<uintptr_t>(param_0)); // PC_PORT: 64-bit pointers
...
bool JASHeap::alloc(JASHeap* mother, u32 param_1) {
    ...
    for (it = mother->mTree.getFirstChild(); it != mother->mTree.getEndChild(); ++it) {
        ...
        u32 local_3c = static_cast<u32>(reinterpret_cast<uintptr_t>(it->mBase) - reinterpret_cast<uintptr_t>(r29)); // PC_PORT
