// =============================================================================
// PC_PORT PATCH of the vendored JSystem/JAudio2/JASAiCtrl.cpp (see
// patches/README.md).
//
// Changes (documented; nothing else differs from the vendored file):
//   - AIInitDMA / JASDsp::syncFrame are given 32-bit *registry tokens* for the
//     host buffers instead of truncated 32-bit addresses (the console ABI);
//     the AI/DSP emulation resolves them. See docs/audio.md §"32-bit pointer
//     ABI".
// =============================================================================
...
void JASDriver::initAI(void (*param_0)(void)) {
    ...
    AIInit(nullptr);
    AIInitDMA(compat::audio::storePtr(sDmaDacBuffer[2]), size); // PC_PORT: token
    BOOL isOutputRate;
    if (sOutputRate == 0) {
        isOutputRate = FALSE;
    } else {
...
void JASDriver::updateDac() {
    ...
    if (r30) {
        AIInitDMA(compat::audio::storePtr(r30), getDacSize() * 2); // PC_PORT: token
    }
...
void JASDriver::finishDSPFrame() {
    ...
    JASDsp::syncFrame(getSubFrames(),
                      compat::audio::storePtr(sDspDacBuffer[sDspDacWriteBuffer]), // PC_PORT: token
                      compat::audio::storePtr(sDspDacBuffer[sDspDacWriteBuffer] + r27));
    sDspStatus = 1;
    updateDSP();
    if (sDspDacCallback) {
