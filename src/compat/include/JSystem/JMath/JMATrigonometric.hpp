...
// PC_PORT: upstream declared `namespace std { struct pair { A1 a1; B1 b1; } }`
// as a stand-in for Metrowerks' standard library. libstdc++/MSVC already ship
// std::pair, so redefining it is a hard error. The table only uses the a1/b1
// member names, so we keep the same struct under a JMath-local name and point
// the table at it (the external TSinCosTable symbol keeps its signature).
namespace JMath {
