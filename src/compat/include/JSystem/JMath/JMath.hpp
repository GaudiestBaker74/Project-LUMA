...
#include <cmath>    // PC_PORT: std::sqrt in the host JMAFastSqrt body
#include <cstring>  // PC_PORT: std::memcpy for the host gekko_ps_copy12/16 bodies
...
#else
    // PC_PORT: the PPC frsqrte estimate * input approximates sqrtf; on the
    // host just use the exact sqrt (same special cases: <= 0 returns input).
    if (input > 0.0f) {
...
inline f32 JMAHermiteInterpolation(__REGISTER f32 p1, __REGISTER f32 p2, __REGISTER f32 p3, __REGISTER f32 p4, __REGISTER f32 p5, __REGISTER f32 p6,
    ...
    return ff25;
#else
    // PC_PORT: straight transcription of the PPC asm above (fnmsubs =
    // dst - fma, fmsubs = fma - product, fmadds = fma + product).
    const f32 ff31 = p1 - p2;
    const f32 ff30 = p5 - p2;
    const f32 ff29 = ff31 / ff30;                       // t
    const f32 ff28 = ff29 * ff29;                       // t^2
    const f32 ff27 = ff28 - ff29;                       // t^2 - t
    const f32 ff26 = (2.0f * ff29 * ff27 - ff28) * (p3 - p6) + p3;
    const f32 ff25 = ff29 * p4 - (p7 * ff27 + p4 * ff27 + p4);
    return ff26 - ff31 * ff25;
#endif
}
...
namespace JMath {
    ...
    inline void gekko_ps_copy12(__REGISTER void* pDest, __REGISTER const void* pSrc) {
        ...
        ;
#else
        // PC_PORT: 12 = 6 paired-single registers = 48 bytes.
        std::memcpy(pDest, pSrc, 48);
#endif
    }
    ...
    inline void gekko_ps_copy16(__REGISTER void* pDest, __REGISTER const void* pSrc) {
        ...
        ;
#else
        // PC_PORT: 16 = 8 paired-single registers = 64 bytes.
        std::memcpy(pDest, pSrc, 64);
#endif
    }
};  // namespace JMath
