...
//
// Change vs. upstream: `long`/`unsigned long` parameter types became
// `s32`/`u32`. Metrowerks for PPC32 had `long` == 32 bits, so the
// decompiled signatures used it for what the ABI treats as u32/s32. On
// LP64 hosts (Linux x86-64/aarch64) `long` is 64 bits, which changes the
// mangled names AND breaks the callers: `readResource(void*, u32, ...)` in
// the .cpp passes `&local_18` (a `u32*`) where an `unsigned long*` would be
// expected. Everything else is identical to upstream.
//
// Additionally, `SDIFileEntry` keeps the upstream declaration but the host
// struct is 24 bytes (not the console's 20) because `mFileData` is a 64-bit
// pointer: the file table cannot be read in place from the big-endian blob.
// The patched JKRMemArchive converts it (see compat/jsystem/RarcHost.h).
// ============================================================================
