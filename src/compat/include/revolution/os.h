...
//
// Change vs. upstream: __OSBusClock/__MEM2End are declared `extern` in the
// non-Metrowerks branch instead of being initialized globals. Upstream
// defines them as initialized globals so the header is self-contained for the
// decomp build, but on a host toolchain that yields one definition per
// translation unit -> duplicate-symbol link errors. Nothing in the compiled
// game code reads them (verified by grep); if that ever changes, compat/os
// will provide the definitions.
//
...
#else
// PC_PORT: extern declarations, see the header comment.
