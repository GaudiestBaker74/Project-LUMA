...
| `nw4r/ut/Color.h` | **M9.5.4 (v6)**: vista `u32` de `ut::Color` canónica `0xRRGGBBAA` en cualquier endianness (upstream reinterpretaba los bytes `r,g,b,a` como palabra: correcto solo en PowerPC BE). Sin el shadow, `DrawQuad`/`CharWriter` → `GXColor1u32` mandaban los colores de vértice con los canales invertidos (los recuadros amarillos del logo del título). `ToU32ref()` eliminados (nadie los usa; una referencia no se puede corregir). |
